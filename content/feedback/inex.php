...
if($met_member_use)$query = "SELECT * FROM $met_parameter where lang='$lang' and  module=8 and class1='$id' and access<='$lystk1958_member_type' order by no_order";
...
$smtp=$met_fd_smtp;
if($met_fd_type!=0){
if(!isset($lystk1958_member_name) || $lystk1958_member_name=='') $lystk1958_member_name=0;
$query = "INSERT INTO $met_feedback SET
					  ...
					  ip                 = '$ip',
					  addtime            = '$addtime',
					  customerid         = '$lystk1958_member_name',
					  lang               = '$lang'";					  
$db->query($query);
$class_1 = $id;
$id=$db->insert_id();
...
if($met_nurse_feed){
	...
	if(maxnurse()<$met_nurse_max){
		...
		$message="æ¨ç½ç«[{$domain}]æ¶å°äºæ°çåé¦ä¿¡æ¯[{$title}]ï¼è¯·å°½å¿«ç»å½ç½ç«åå°æ¥ç";
...
$query = "SELECT * FROM $met_parameter where lang='$lang' and  module=8 and class1='$id' order by no_order";
if($met_member_use)$query = "select * from $met_parameter where (access in(select id from $met_admin_array where user_webpower<='$lystk1958_member_type') or access=0) and lang='$lang' and module=8 and class1='$id' order by no_order;";
$result = $db->query($query);
while($list= $db->fetch_array($result)){
 if($list[type]==2 or $list[type]==4 or $list[type]==6){
	...
	if($listinfo){
	$listmarknow='lystk1958';
	$classtype=($listinfo[info]=='lystk1958all')?$listinfoid:($met_class[$listinfoid][releclass]?'class1':'class'.$class_list[$listinfoid][classtype]);
